...
//Feryx_starfield_create_Color_different(0,0,1024,1024,30,10,random(255));
//colorized stars ONE COLOR 
horizontal_speed
vertical_speed
horizontal_texture
Vertical_texture
Stars_density
Stars_Maximal_Size
multiple_Different_Hvs_Color (make_color_hvs(255,255,255))
*/
//speeds
sph=argument0;
spv=argument1;
...
for (i = 0; i < l1; i += 1)
{
    a=random(hs);
    b=random(vs);
    c=make_colour_hsv(argument6,random(255),255);
...
for (i = 0; i < l2; i += 1)
{
    ...
    if(multiplee!=1){
    c=make_colour_hsv(argument6,random(255),255);}
    else{c=make_colour_hsv(random(255),random(255),255);}
    d=round(random(starsize));
...
__background_set( e__BG.X, 2, round(random(hs )))
__background_set( e__BG.Y, 2, round(random(vs )))

__background_set( e__BG.X, 3, round(random(hs )))
__background_set( e__BG.Y, 3, round(random(vs )))

__background_set( e__BG.X, 4, round(random(hs )))
__background_set( e__BG.Y, 4, round(random(vs )))

__background_set( e__BG.X, 5, round(random(hs )))
__background_set( e__BG.Y, 5, round(random(vs )))

__background_set( e__BG.X, 6, round(random(hs )))
__background_set( e__BG.Y, 6, round(random(vs )))
